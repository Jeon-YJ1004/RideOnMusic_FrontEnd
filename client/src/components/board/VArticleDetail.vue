...
<template>
  ...
  <div class="row">
    ...
    <div class="d-flex justify-content-end">
      <button
        ...
        @click="router.push({ name: `${boardType}boardlist` })"
      >
        글목록
      </button>
      ...
      <button
        type="button"
        id="btn-mv-modify"
        v-if="article.memberId == loginId"
        class="btn mini btn-outline-success mb-3 ms-1"
        @click="router.push({ name: `${boardType}articlemodify` })"
      >
        글수정
      </button>
      <button
        ...
        id="btn-delete"
        v-if="article.memberId == loginId"
        class="btn mini btn-outline-danger mb-3 ms-1"
        @click="deleteArticle"
      >
        글삭제
      </button>
  ...
  <!-- //TODO - 댓글 작성 폼 -->
  <VCommentForm
    v-if="boardType == '' || (boardType == 'qna' && article.memberId == loginId)"
    :articleNo="articleNo"
    parentId="-1"
